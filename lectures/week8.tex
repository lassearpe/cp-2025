...
\begin{frame}[fragile]
  ...
  \item[\faBook]<2-> Many applications (e.g., web servers, databases, operating systems) rely on handling multiple tasks at once.
  \item[\faBook]<3-> Understanding concurrency helps in writing efficient and
    bug-free programs.
  \end{itemize}
  \vspace{0.6cm}
  \uncover<4->{
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
  \item[\faVolumeUp] Lectures, with a few lectures taking place online through
...
\begin{frame}[fragile]
  \frametitle{Examples of Concurrency, one chef}
...
\begin{frame}[fragile]
  \frametitle{Examples of Concurrency, one }

  \large{
  \begin{itemize}
  \item[\faBook]<1-> You don't need more than one chef/CPU cores/threads to have
    concurrent execution.
  \item[\faBook]<2-> \textbf{Task switching}/\textbf{interleaving} are the hallmarks of
    concurrency.
  \end{itemize}}
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Examples of Concurrency, two chefs}
...
\begin{frame}[fragile]
  ...
  \large{
    \begin{itemize}
    \item[\faBook]<1-> Again, there are actually distinct chefs/\textbf{threads} of
      execution taking place.
    \item[\faBook]<1-> However, their tasks are distinct, and there is no
      communication between the two.
    \item[\faBook]<2-> This is \textbf{parallel programming}, a different topic
